...
class ThermalRelaxationChannel(ErrorChannel):
    ...
    def kraus_operators(self) -> List[np.ndarray]:
        ...
        # Combined amplitude + phase damping using correct Kraus decomposition.
        # The CPTP condition requires: K0†K0 + K1†K1 + K2†K2 = I
        # K0 = |0><0| + sqrt(1-g)*sqrt(1-lam)*|1><1|
        # K1 = sqrt(g)*|0><1|
        # K2 = sqrt(1-g)*sqrt(lam)*|1><1|
        # Check: K0†K0 = |0><0| + (1-g)(1-lam)|1><1|
        #        K1†K1 = g|1><1|
        #        K2†K2 = (1-g)*lam|1><1|
        #        Sum = |0><0| + [(1-g)(1-lam) + g + (1-g)lam]|1><1|
        #            = |0><0| + [(1-g) - (1-g)lam + g + (1-g)lam]|1><1|
        #            = |0><0| + [1-g + g]|1><1|
        #            = |0><0| + |1><1| = I  ✓
        sqrt_1mg_sqrt_1ml = sqrt_1mg * np.sqrt(max(0, 1 - lam))
        sqrt_1mg_sqrt_lam = sqrt_1mg * np.sqrt(max(0, lam))

        K0 = np.array([[1, 0], [0, sqrt_1mg_sqrt_1ml]], dtype=np.complex128)
        K1 = np.array([[0, sqrt_g], [0, 0]], dtype=np.complex128)
        K2 = np.array([[0, 0], [0, sqrt_1mg_sqrt_lam]], dtype=np.complex128)
